...
GH_COOKIE     = os.environ.get("GH_COOKIE") or ""       # GitHub 网页会话 Cookie, SESSION_TOKEN 失效时自动 OAuth 登录(GitHub 登录的账号用)
...
if not SESSION_TOKEN and not DC_TOKEN and not GH_COOKIE:
    print("ℹ️ 未配置 SESSION_TOKEN、DISCORD_TOKEN 和 GH_COOKIE,脚本终止。")
    sys.exit(1)
...
def do_github_login(sb) -> bool:
    ...
    print("\n🔑 通过 GitHub Cookie 登录...")
    if not GH_COOKIE:
        print("⚠️ 未配置 GH_COOKIE，跳过 GitHub 登录")
        return False
    ...
    time.sleep(2)
    for c in parse_cookie_string(GH_COOKIE):
    ...
    if "/login" in sb.get_current_url().lower():
        print("❌ GitHub Cookie 无效或已过期，请更新 GH_COOKIE")
